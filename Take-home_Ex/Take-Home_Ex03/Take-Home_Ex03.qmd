...
As the computation time for the original training data set takes too long on my computer, I have decided to scale down the training data to the latest 3 months of data (October to December 2022), so that the I can still calibrate a model and do the prediction.

```{r}
train_data_scaled <- train_data %>% filter(month >= "2022-10" & month <= "2022-12")
```
...
![](images/image-836409430.png)

The results show that 24 neighbour points will be the optimal bandwidth to be used for the adaptive bandwidth.
...
![](images/image-1589230712.png)

![](images/image-920137067.png)

![](images/image-748787081.png)
...
We can also use a scatterpolot to visualise the actual resale price and predicted resale price.
